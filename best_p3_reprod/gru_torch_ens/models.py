...
class BaselineGruModel(nn.Module):
    ...
    def __init__(self, settings):
        ...
        ## GNN pretreatment
        # self.graph_w = settings['graph_w']
        # self.graph_h = settings['graph_h']
        if self.use_conv:
            self.covn1 = nn.Conv2d(in_channels=1, out_channels=10, kernel_size=(2,2),padding=1)
            self.covn2 = nn.Conv2d(in_channels=10, out_channels=1, kernel_size=(2,2),padding=0)
        #######################
    ...
    def forward(self, numerical_features, categorical_features=None, output_len=288):
        ...
        # Graph 
        if self.use_conv:
            numerical_features = torch.reshape(numerical_features,(-1,1,self.input_len,self.hidC))
            x0 = self.covn1(numerical_features)
            x1 = self.covn2(x0)
            # print(x1.size())
            numerical_features = torch.reshape(x1,(-1,self.input_len,self.hidC))
        ##################################################
